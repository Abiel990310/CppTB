...
const LANGS = ['cpp', 'c', 'bash', 'cmake', 'json', 'yaml', 'diff', 'makefile', 'text'];
 ...
 *
 *     ```cpp run std=c++23 title="Hello, world"
 *
 * `run` adds a compile-and-run panel, `asm` adds an assembly panel, and
...
function parseFenceInfo(info: string): FenceMeta {
  ...
  meta.run = meta.flags.has('run');
  meta.asm = meta.flags.has('asm');
  return meta;
}
...
function containerPlugin(md: MarkdownIt): void {
  ...
  md.renderer.rules.container_open = (tokens, idx) => {
    ...
    if (name === 'memviz') {
      return `<figure class="widget"><cpp-memviz><template data-role="spec">${escapeHtml(body ?? '')}</template></cpp-memviz></figure>\n`;
    }
    if (name === 'exercise') {
      return `<cpp-exercise data-id="${escapeHtml(args)}"></cpp-exercise>\n`;
    }
    if (name === 'quiz') {
      return `<cpp-quiz><template data-role="spec">${escapeHtml(body ?? '')}</template></cpp-quiz>\n`;
    }
...
export async function createRenderer(): Promise<(src: string) => RenderResult> {
  ...
  md.renderer.rules.fence = (tokens, idx) => {
    ...
    return (
      `<figure class="widget"><cpp-runner ${attrs}>` +
      `${caption}` +
